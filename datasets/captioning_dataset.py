...
def caption_iterator(cfg, batch_size, phase):
    ...
    metapath = "./data/merge.csv"
    vocab_dataset = data.TabularDataset(
        path=metapath, format='tsv', skip_header=True, fields=fields,
    )

    CAPTION.build_vocab(vocab_dataset.caption,
                        min_freq=cfg.min_freq_caps, vectors=cfg.word_emb_caps)
